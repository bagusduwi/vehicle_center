    ...
    <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
        <div class="widget-content widget-content-area br-8">
            <table id="zero-config" class="table table-striped dt-table-hover">
                ...
                <tbody>
                    @foreach($fuel as $row => $data)
                    <tr>
                        <td class="text-center">{{$row + 1}}</td>
                        <td class="text-center">{{$data->vehicle_name}}</td>
                        <td class="text-center">{{$data->consumtion}}</td>
                        <td class="text-center">{{$data->date}}</td>
                        <td class="text-center">
                            <a href="{{url('/fuel/'.$data->id.'/edit')}}" class="bs-tooltip" data-bs-toggle="tooltip"
                            ...
                            <form action="{{url('fuel/'.$data->id)}}" method="post" style="display: inline-block;">
                                @csrf
                                @method('delete')
